...
function getPaper(id) {
    return $next().get("/api/question/user-exam-paper/" + id + "?details");
}
...
function submitAnswer(id, answers, force = false) {
    return $next().post(`/api/question/user-exam-paper/${id}/i-finish-all${force ? "?force" : ""}`, answers);
}
...
function submitQuestionAnswer(id, answers) {
    return $next().post(`/api/question/user-exam-paper/q/${id}/answer`, answers);
}

function getAnswer(id) {
    return $next().post(`/api/question/user-exam-paper/${id}/i-need-answer`);
}
...
function checkPaper(id, action) {
    return $next().put("/api/question/exam-paper/" + id + "/opt", "", {
        params: {
            action: action,
        },
